...
#
RELEASE       := 45
RELEASE_PATH  := http://embedxcode.weebly.com/release.html
RELEASE_LAST  := $(shell curl -s $(RELEASE_PATH) | grep -e '>release=' | cut -d'>' -f 2 | cut -d'<' -f 1 | cut -d'=' -f 2)
...
prepare:
		@if [ -f Utilities/embedXcode_prepare ]; then Utilities/embedXcode_prepare $(PROJECT_FILE_PATH); rm -r Utilities/embedXcode_prepare; if [ $(RELEASE_LAST) -gt $(RELEASE) ]; then osascript -e 'tell application "System Events" to if button returned of (display dialog "A new release $(RELEASE_LAST) is available.\n\nInstalled release is $(RELEASE)." buttons {"Go to Download", "OK"} with icon POSIX file ("$(UTILITIES_PATH)/TemplateIcon.icns") default button 2 with title "embedXcode" giving up after 5) = "Go to Download" then tell application "Safari"' -e 'open location "http://www.embedXcode.weebly.com/download"' -e 'activate' -e 'end tell'; fi; fi;
		@if [ $(RELEASE_LAST) -gt $(RELEASE) ]; then echo "New release $(RELEASE_LAST) available, installed $(RELEASE)."; fi;
